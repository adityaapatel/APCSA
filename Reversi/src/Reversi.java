...
public class Reversi
{
	...
	//**********************************************************************
	{
		...
		System.out.println("\t\t   Author: Aditya Patel\n\n"); // <-- Insert Your Name Here!
	...
	//**********************************************************************
	{
		...
		public static void getFakeWinner(int [][]j)
	{
		int scoreX= 0;
		int scoreO= 0;
		int winner= -1;
	...
	//** If a corner is available, the corners will be taken, if not the best valid move will be taken.
	//**********************************************************************
	{
		
//		int bestMove = -1;
//		String Winner= "";
//		 int bTotal = pieceCount(BLACK);
//		int wTotal = pieceCount(WHITE);
//		int[][] testArray= board.clone();
//	        
//	        for(int row = testArray.length-1; row >= 0;row--)//ITERATE Backwards
//			{	
//				 for(int col = 0; col < testArray.length;col++)  // Iterate Forwards
//				{
//		
//					 if (testArray[row][col]== 0 && checkMove(col, row, playerColor))
//			
//					 {
//						 putPiece(col, row, playerColor); // if corner is available, place at available corner.
//						 getFakeWinner(testArray);
//	
//					 }
//			
//		
...
//			}
		...
		int r= 0;
		mover:
		//for(int row = 0; row < board.length;row++) //Iterate Forwards
		//  for (int col =board.length-1; col>=0; col--) //ITERATE Backwards
			
			for(int row = board.length-1; row >= 0;row--)//ITERATE Backwards
		{	
			 for(int col = 0; col < board.length;col++)  // Iterate Forwards
			{
				r= row;
				if(/*board[row][col]== 0*/  checkMove(col, row, playerColor))
					
				{	
						if (row== 7 &&col ==7)
						{
							putPiece(7, 7, playerColor); // if corner is available, place at available corner.
						}
						else
						if (row== 0 &&col ==0)
						{
							putPiece(0, 0, playerColor); // if corner is available, place at available corner.
						}
						else
						 if (row== 0 &&col ==7)
						{
							putPiece(7,0, playerColor); // if corner is available, place at available corner.
						}
						else
						if (row== 7 && col ==0)
						{
							putPiece(0, 7, playerColor); // if corner is available, place at available corner.
						}
						...
						//**********************************************************************
						//** Random Moves
						//** - Picks a Random move on the array and checks if the move is valid and goes there,
						//**	
						//**********************************************************************	
							
//					Random random= new Random();
//					int rCol= random.nextInt(board.length);
//					int rRow= random.nextInt(board.length);
//					
//					while(rCol==col)
//					{
//						
//						 if( checkMove(rCol, rRow, playerColor) ) 
//						 {
//							 	
//						 }
//						 else 
...
//						
					...
					// The following switch statement gets the value of col and changes the numerical slot too a letter slot.
					switch(col) 
					{
					case 0:
						letterCol = "A";
					...
					case 7:
						...
						break;
						default: 
							letterCol ="Error"; // Error results in error
							break;
					}
					break mover;
				}
		...
		System.out.println("Computer placed peice at location: " + letterCol + (r+1)); // Returns the slot of placement.
